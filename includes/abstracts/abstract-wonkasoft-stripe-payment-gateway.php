...
abstract class Wonkasoft_Stripe_WC_Payment_Gateway_CC extends WC_Payment_Gateway_CC {
	...
	public function save_payment_method_checkbox() {
		printf(
			...
			esc_attr( $this->id ),
			esc_html( apply_filters( 'wc_stripe_save_to_account_text', __( 'Save payment information to my account for future purchases.', 'wonkasoft-stripe' ) ) )
		);
	}
	...
	public function maybe_process_pre_orders( $order_id ) {
		return (
			Wonkasoft_Stripe_Helper::is_pre_orders_exists() &&
	...
	public function payment_icons() {
		return apply_filters(
			'wonkasoft_wc_stripe_payment_icons',
			array(
				'visa'       => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/visa.svg" class="stripe-visa-icon stripe-icon" alt="Visa" />',
				'amex'       => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/amex.svg" class="stripe-amex-icon stripe-icon" alt="American Express" />',
				'mastercard' => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/mastercard.svg" class="stripe-mastercard-icon stripe-icon" alt="Mastercard" />',
				'discover'   => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/discover.svg" class="stripe-discover-icon stripe-icon" alt="Discover" />',
				// 'diners'     => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/diners.svg" class="stripe-diners-icon stripe-icon" alt="Diners" />',
				// 'jcb'        => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/jcb.svg" class="stripe-jcb-icon stripe-icon" alt="JCB" />',
				'alipay'     => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/alipay.svg" class="stripe-alipay-icon stripe-icon" alt="Alipay" />',
				'wechat'     => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/wechat.svg" class="stripe-wechat-icon stripe-icon" alt="Wechat Pay" />',
				'bancontact' => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/bancontact.svg" class="stripe-bancontact-icon stripe-icon" alt="Bancontact" />',
				'ideal'      => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/ideal.svg" class="stripe-ideal-icon stripe-icon" alt="iDeal" />',
				'p24'        => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/p24.svg" class="stripe-p24-icon stripe-icon" alt="P24" />',
				'giropay'    => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/giropay.svg" class="stripe-giropay-icon stripe-icon" alt="Giropay" />',
				'eps'        => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/eps.svg" class="stripe-eps-icon stripe-icon" alt="EPS" />',
				'multibanco' => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/multibanco.svg" class="stripe-multibanco-icon stripe-icon" alt="Multibanco" />',
				'sofort'     => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/sofort.svg" class="stripe-sofort-icon stripe-icon" alt="SOFORT" />',
				'sepa'       => '<img src="' . WONKASFT_STRIPE_PLUGIN_URL . '/assets/images/sepa.svg" class="stripe-sepa-icon stripe-icon" alt="SEPA" />',
			)
		);
	}
	...
	public function get_stripe_customer_id( $order ) {
		$customer = get_user_meta( Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->customer_user : $order->get_customer_id(), '_stripe_customer_id', true );
	...
	public function generate_payment_request( $order, $prepared_source ) {
		...
		$post_data             = array();
		$post_data['currency'] = strtolower( Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->get_order_currency() : $order->get_currency() );
		$post_data['amount']   = Wonkasoft_Stripe_Helper::get_stripe_amount( $order->get_total(), $post_data['currency'] );
		/* translators: 1) blog name 2) order number */
		$post_data['description'] = sprintf( __( '%1$s - Order %2$s', 'wonkasoft-stripe' ), wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ), $order->get_order_number() );
		$billing_email            = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_email : $order->get_billing_email();
		$billing_first_name       = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_first_name : $order->get_billing_first_name();
		$billing_last_name        = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_last_name : $order->get_billing_last_name();

		if ( ! empty( $billing_email ) && apply_filters( 'wc_stripe_send_stripe_receipt', false ) ) {
			$post_data['receipt_email'] = $billing_email;
		}

		switch ( Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->payment_method : $order->get_payment_method() ) {
			case 'stripe':
				if ( ! empty( $statement_descriptor ) ) {
					$post_data['statement_descriptor'] = Wonkasoft_Stripe_Helper::clean_statement_descriptor( $statement_descriptor );
				}
		...
		$metadata = array(
			__( 'customer_name', 'wonkasoft-stripe' )  => sanitize_text_field( $billing_first_name ) . ' ' . sanitize_text_field( $billing_last_name ),
			__( 'customer_email', 'wonkasoft-stripe' ) => sanitize_email( $billing_email ),
			'order_id'                                 => $order->get_order_number(),
		);

		if ( $this->has_subscription( Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->id : $order->get_id() ) ) {
			$metadata += array(
	...
	public function process_response( $response, $order ) {
		...
		// Store charge data.
		Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, '_stripe_charge_captured', $captured ) : $order->update_meta_data( '_stripe_charge_captured', $captured );
		...
		if ( 'yes' === $captured ) {
			...
			if ( 'pending' === $response->status ) {
				$order_stock_reduced = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, '_order_stock_reduced', true ) : $order->get_meta( '_order_stock_reduced', true );

				if ( ! $order_stock_reduced ) {
					Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->reduce_order_stock() : wc_reduce_stock_levels( $order_id );
				}
				...
				$order->update_status( 'on-hold', sprintf( __( 'Stripe charge awaiting payment: %s.', 'wonkasoft-stripe' ), $response->id ) );
			}
			...
			if ( 'succeeded' === $response->status ) {
				...
				/* translators: transaction id */
				$message = sprintf( __( 'Stripe charge complete (Charge ID: %s)', 'wonkasoft-stripe' ), $response->id );
			...
			/* translators: transaction id */
			$order->update_status( 'on-hold', sprintf( __( 'Stripe charge authorized (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization.', 'wonkasoft-stripe' ), $response->id ) );
		}
	...
	public function get_owner_details( $order ) {
		...
		$email = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_email : $order->get_billing_email();
		$phone = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_phone : $order->get_billing_phone();
		...
		$details['address']['line1']       = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_address_1 : $order->get_billing_address_1();
		$details['address']['line2']       = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_address_2 : $order->get_billing_address_2();
		$details['address']['state']       = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_state : $order->get_billing_state();
		$details['address']['city']        = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_city : $order->get_billing_city();
		$details['address']['postal_code'] = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_postcode : $order->get_billing_postcode();
		$details['address']['country']     = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->billing_country : $order->get_billing_country();
	...
	public function prepare_order_source( $order = null ) {
		...
		if ( $order ) {
			...
			$source_id = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, '_stripe_source_id', true ) : $order->get_meta( '_stripe_source_id', true );

			// Since 4.0.0, we changed card to source so we need to account for that.
			if ( empty( $source_id ) ) {
				$source_id = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, '_stripe_card_id', true ) : $order->get_meta( '_stripe_card_id', true );

				// Take this opportunity to update the key name.
				Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, '_stripe_source_id', $source_id ) : $order->update_meta_data( '_stripe_source_id', $source_id );
	...
	public function update_fees( $order, $balance_transaction_id ) {
		...
		if ( empty( $balance_transaction->error ) ) {
			if ( isset( $balance_transaction ) && isset( $balance_transaction->fee ) ) {
				...
				$fee_refund = ! empty( $balance_transaction->fee ) ? Wonkasoft_Stripe_Helper::format_balance_fee( $balance_transaction, 'fee' ) : 0;
				$net_refund = ! empty( $balance_transaction->net ) ? Wonkasoft_Stripe_Helper::format_balance_fee( $balance_transaction, 'net' ) : 0;

				// Current data fee & net.
				$fee_current = Wonkasoft_Stripe_Helper::get_stripe_fee( $order );
				$net_current = Wonkasoft_Stripe_Helper::get_stripe_net( $order );

				// Calculation.
				$fee = (float) $fee_current + (float) $fee_refund;
				$net = (float) $net_current + (float) $net_refund;

				Wonkasoft_Stripe_Helper::update_stripe_fee( $order, $fee );
				Wonkasoft_Stripe_Helper::update_stripe_net( $order, $net );

				$currency = ! empty( $balance_transaction->currency ) ? strtoupper( $balance_transaction->currency ) : null;
				Wonkasoft_Stripe_Helper::update_stripe_currency( $order, $currency );
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( ! is_null( $amount ) ) {
			$request['amount'] = Wonkasoft_Stripe_Helper::get_stripe_amount( $amount, $order_currency );
		}
		...
		if ( ! empty( $response->error ) ) {
			...
			Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, '_stripe_refund_id', $response->id ) : $order->update_meta_data( '_stripe_refund_id', $response->id );

			$amount = wc_price( $response->amount / 100 );

			if ( in_array( strtolower( Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->get_order_currency() : $order->get_currency() ), Wonkasoft_Stripe_Helper::no_decimal_currencies() ) ) {
			...
			$refund_message = ( isset( $captured ) && 'yes' === $captured ) ? sprintf( __( 'Refunded %1$s - Refund ID: %2$s - Reason: %3$s', 'wonkasoft-stripe' ), $amount, $response->id, $reason ) : __( 'Pre-Authorization Released', 'wonkasoft-stripe' );
	...
	public function add_payment_method() {
		$error     = false;
		$error_msg = __( 'There was a problem adding the payment method.', 'wonkasoft-stripe' );
		$source_id = '';
	...
	public function generate_create_intent_request( $order, $prepared_source ) {
		...
		$request = array(
			...
			'amount'               => Wonkasoft_Stripe_Helper::get_stripe_amount( $order->get_total() ),
			'currency'             => strtolower( Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->get_order_currency() : $order->get_currency() ),
	...
	public function update_existing_intent( $intent, $order, $prepared_source ) {
		...
		$new_amount = Wonkasoft_Stripe_Helper::get_stripe_amount( $order->get_total() );
	...
	public function create_and_confirm_intent_for_off_session( $order, $prepared_source, $amount = null ) {
		...
		$request = array(
			'amount'               => $amount ? Wonkasoft_Stripe_Helper::get_stripe_amount( $amount, $full_request['currency'] ) : $full_request['amount'],
