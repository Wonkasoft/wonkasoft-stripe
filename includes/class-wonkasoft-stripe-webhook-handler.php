...
class Wonkasoft_Stripe_Webhook_Handler extends Wonkasoft_Stripe_WC_Payment_Gateway_CC {
	...
	public function check_for_webhook() {
		...
		if ( $this->is_valid_request( $request_headers, $request_body ) ) {
			...
			Wonkasoft_Stripe_Logger::log( 'Incoming webhook failed validation: ' . print_r( $request_body, true ) );
			status_header( 400 );
			exit;
		}
	}
	...
	public function process_webhook_payment( $notification, $retry = true ) {
		...
		$order = Wonkasoft_Stripe_Helper::get_order_by_source_id( $notification->data->object->id );

		if ( ! $order ) {
			Wonkasoft_Stripe_Logger::log( 'Could not find order via source ID: ' . $notification->data->object->id );
			return;
		}
		...
		try {
			...
			Wonkasoft_Stripe_Logger::log( "Info: (Webhook) Begin processing payment for order $order_id for the amount of {$order->get_total()}" );
			...
			if ( ! empty( $response->error ) ) {
				...
				$localized_messages = Wonkasoft_Stripe_Helper::get_localized_messages();
	...
	public function process_webhook_dispute( $notification ) {
		$order = Wonkasoft_Stripe_Helper::get_order_by_charge_id( $notification->data->object->charge );

		if ( ! $order ) {
			Wonkasoft_Stripe_Logger::log( 'Could not find order via charge ID: ' . $notification->data->object->charge );
			return;
		}
	...
	public function process_webhook_capture( $notification ) {
		$order = Wonkasoft_Stripe_Helper::get_order_by_charge_id( $notification->data->object->id );

		if ( ! $order ) {
			Wonkasoft_Stripe_Logger::log( 'Could not find order via charge ID: ' . $notification->data->object->id );
			return;
		}

		$order_id = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->id : $order->get_id();

		if ( 'stripe' === ( Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->payment_method : $order->get_payment_method() ) ) {
			$charge   = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, '_transaction_id', true ) : $order->get_transaction_id();
			$captured = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, '_stripe_charge_captured', true ) : $order->get_meta( '_stripe_charge_captured', true );

			if ( $charge && 'no' === $captured ) {
				Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, '_stripe_charge_captured', 'yes' ) : $order->update_meta_data( '_stripe_charge_captured', 'yes' );

				// Store other data such as fees
				Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, '_transaction_id', $notification->data->object->id ) : $order->set_transaction_id( $notification->data->object->id );
	...
	public function process_webhook_source_canceled( $notification ) {
		...
		if ( ! $order ) {
			...
			if ( ! $order ) {
				Wonkasoft_Stripe_Logger::log( 'Could not find order via charge/source ID: ' . $notification->data->object->id );
				return;
			}
		}
		...
		if ( 'stripe' !== $order->get_payment_method() ) {
			Wonkasoft_Stripe_Logger::log( 'Canceled webhook abort: Order was not processed by Stripe: ' . $order->get_id() );
			return;
		}
	...
	public function process_webhook_refund( $notification ) {
		...
		if ( 'stripe' === ( Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->payment_method : $order->get_payment_method() ) ) {
			$charge    = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, '_transaction_id', true ) : $order->get_transaction_id();
			$captured  = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, '_stripe_charge_captured', true ) : $order->get_meta( '_stripe_charge_captured', true );
			$refund_id = Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, '_stripe_refund_id', true ) : $order->get_meta( '_stripe_refund_id', true );
			...
			if ( $charge ) {
				...
				if ( is_wp_error( $refund ) ) {
					Wonkasoft_Stripe_Logger::log( $refund->get_error_message() );
				}

				Wonkasoft_Stripe_Helper::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, '_stripe_refund_id', $notification->data->object->refunds->data[0]->id ) : $order->update_meta_data( '_stripe_refund_id', $notification->data->object->refunds->data[0]->id );
	...
	public function process_review_opened( $notification ) {
		if ( isset( $notification->data->object->payment_intent ) ) {
			$order = Wonkasoft_Stripe_Helper::get_order_by_intent_id( $notification->data->object->payment_intent );

			if ( ! $order ) {
				Wonkasoft_Stripe_Logger::log( '[Review Opened] Could not find order via intent ID: ' . $notification->data->object->payment_intent );
				return;
			}
		} else {
			$order = Wonkasoft_Stripe_Helper::get_order_by_charge_id( $notification->data->object->charge );
			...
			if ( ! $order ) {
				Wonkasoft_Stripe_Logger::log( '[Review Opened] Could not find order via charge ID: ' . $notification->data->object->charge );
				return;
			}
		}
	...
	public function process_review_closed( $notification ) {
		if ( isset( $notification->data->object->payment_intent ) ) {
			...
			if ( ! $order ) {
				Wonkasoft_Stripe_Logger::log( '[Review Closed] Could not find order via intent ID: ' . $notification->data->object->payment_intent );
				return;
			}
		} else {
			$order = Wonkasoft_Stripe_Helper::get_order_by_charge_id( $notification->data->object->charge );
			...
			if ( ! $order ) {
				Wonkasoft_Stripe_Logger::log( '[Review Closed] Could not find order via charge ID: ' . $notification->data->object->charge );
				return;
			}
		}
	...
	public function get_refund_amount( $notification ) {
		if ( $this->is_partial_capture( $notification ) ) {
			...
			if ( in_array( strtolower( $notification->data->object->currency ), Wonkasoft_Stripe_Helper::no_decimal_currencies() ) ) {
	...
	public function process_payment_intent_success( $notification ) {
		...
		$order  = Wonkasoft_Stripe_Helper::get_order_by_intent_id( $intent->id );

		if ( ! $order ) {
			Wonkasoft_Stripe_Logger::log( 'Could not find order via intent ID: ' . $intent->id );
			return;
		}
	...
	public function process_setup_intent( $notification ) {
		$intent = $notification->data->object;
		$order  = Wonkasoft_Stripe_Helper::get_order_by_setup_intent_id( $intent->id );

		if ( ! $order ) {
			Wonkasoft_Stripe_Logger::log( 'Could not find order via setup intent ID: ' . $intent->id );
			return;
		}
		...
		if ( 'setup_intent.succeeded' === $notification->type ) {
			Wonkasoft_Stripe_Logger::log( "Stripe SetupIntent $intent->id succeeded for order $order_id" );
			if ( Wonkasoft_Stripe_Helper::is_pre_orders_exists() && WC_Pre_Orders_Order::order_contains_pre_order( $order ) ) {
