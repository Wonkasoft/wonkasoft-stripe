...
class Wonkasoft_Stripe_Helper {
	...
	public static function get_stripe_currency( $order = null ) {
		...
		$order_id = self::is_wc_lt( '3.0' ) ? $order->id : $order->get_id();

		return self::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::META_NAME_STRIPE_CURRENCY, true ) : $order->get_meta( self::META_NAME_STRIPE_CURRENCY, true );
	}
	...
	public static function update_stripe_currency( $order = null, $currency ) {
		...
		self::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, self::META_NAME_STRIPE_CURRENCY, $currency ) : $order->update_meta_data( self::META_NAME_STRIPE_CURRENCY, $currency );
	}
	...
	public static function get_stripe_fee( $order = null ) {
		...
		$amount = self::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::META_NAME_FEE, true ) : $order->get_meta( self::META_NAME_FEE, true );

		// If not found let's check for legacy name.
		if ( empty( $amount ) ) {
			$amount = self::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::LEGACY_META_NAME_FEE, true ) : $order->get_meta( self::LEGACY_META_NAME_FEE, true );
	...
	public static function update_stripe_fee( $order = null, $amount = 0.0 ) {
		...
		self::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, self::META_NAME_FEE, $amount ) : $order->update_meta_data( self::META_NAME_FEE, $amount );
	}
	...
	public static function get_stripe_net( $order = null ) {
		...
		$amount = self::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::META_NAME_NET, true ) : $order->get_meta( self::META_NAME_NET, true );

		// If not found let's check for legacy name.
		if ( empty( $amount ) ) {
			$amount = self::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::LEGACY_META_NAME_NET, true ) : $order->get_meta( self::LEGACY_META_NAME_NET, true );
	...
	public static function update_stripe_net( $order = null, $amount = 0.0 ) {
		...
		self::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, self::META_NAME_NET, $amount ) : $order->update_meta_data( self::META_NAME_NET, $amount );
	}
	...
	public static function get_localized_messages() {
		return apply_filters(
			'wonkasoft_stripe_localized_messages',
			array(
				'invalid_number'           => __( 'The card number is not a valid credit card number.', 'wonkasoft-stripe' ),
				'invalid_expiry_month'     => __( 'The card\'s expiration month is invalid.', 'wonkasoft-stripe' ),
				'invalid_expiry_year'      => __( 'The card\'s expiration year is invalid.', 'wonkasoft-stripe' ),
				'invalid_cvc'              => __( 'The card\'s security code is invalid.', 'wonkasoft-stripe' ),
				'incorrect_number'         => __( 'The card number is incorrect.', 'wonkasoft-stripe' ),
				'incomplete_number'        => __( 'The card number is incomplete.', 'wonkasoft-stripe' ),
				'incomplete_cvc'           => __( 'The card\'s security code is incomplete.', 'wonkasoft-stripe' ),
				'incomplete_expiry'        => __( 'The card\'s expiration date is incomplete.', 'wonkasoft-stripe' ),
				'expired_card'             => __( 'The card has expired.', 'wonkasoft-stripe' ),
				'incorrect_cvc'            => __( 'The card\'s security code is incorrect.', 'wonkasoft-stripe' ),
				'incorrect_zip'            => __( 'The card\'s zip code failed validation.', 'wonkasoft-stripe' ),
				'invalid_expiry_year_past' => __( 'The card\'s expiration year is in the past', 'wonkasoft-stripe' ),
				'card_declined'            => __( 'The card was declined.', 'wonkasoft-stripe' ),
				'missing'                  => __( 'There is no card on a customer that is being charged.', 'wonkasoft-stripe' ),
				'processing_error'         => __( 'An error occurred while processing the card.', 'wonkasoft-stripe' ),
				'invalid_request_error'    => __( 'Unable to process this payment, please try again or use alternative method.', 'wonkasoft-stripe' ),
				'invalid_sofort_country'   => __( 'The billing country is not accepted by SOFORT. Please try another country.', 'wonkasoft-stripe' ),
				'email_invalid'            => __( 'Invalid email address, please correct and try again.', 'wonkasoft-stripe' ),
			)
		);
	}
