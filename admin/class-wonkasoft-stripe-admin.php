...
class Wonkasoft_Stripe_Admin {
	...
	public function wonkasoft_action_link_filter( $links ) {
		...
		$links[] = '<a href="https://paypal.me/Wonkasoft" target="blank"><img src="' . plugins_url( '/img/wonka-logo.svg', __FILE__ ) . '" style="width: 20px; height: 20px; display: inline-block; vertical-align: middle; float: none;" /></a>';
		return $links;
	}
	...
	public function wonkasoft_plugin_row_meta_filter( $links, $file ) {
		if ( strpos( $file, 'wonkasoft-stripe.php' ) !== false ) {
			$links[] = '<a href="' . get_admin_url( null, 'admin.php?page=wc-settings&tab=checkout&section=wonkasoft_stripe' ) . '" target="_self">Settings</a>';
			$links[] = '<a href="https://paypal.me/Wonkasoft" target="blank">Donate <img src="' . plugins_url( '/img/wonka-logo.svg', __FILE__ ) . '" style="width: 20px; height: 20px; display: inline-block; vertical-align: middle;" /></a>';
		}
		return $links;
	}
}
